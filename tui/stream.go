...
func (m *Model) contextSections(ragBlock string) []contextSection {
	...
	case WriteMode:
		add("mode rules", "WRITE: full toolset. You may modify files and run any shell command. Each destructive call surfaces a permission prompt the user must approve. Work from the plan in your session notes, but verify each step against the ACTUAL code as you execute it — don't assume the note is still accurate. If the code contradicts the plan or notes, trust the code, say so, and adjust. You can switch_mode back to 'plan' or 'explore' if you discover the plan is wrong. When the plan is fully executed, call switch_mode(\"verify\", ...) so the changes get reviewed.\n")
	case VerifyMode:
		add("mode rules", "VERIFY: you are an adversarial code reviewer. Assume the write-mode changes are wrong until the code proves otherwise; your job is to find what is broken or sloppy, not to approve. No file writes; run_shell is limited to the explore read-only allowlist plus go test, which needs the user's approval. Read the full diff (git_diff, git_status) and every touched file, line by line — never skim, never trust the plan or the author's summary. Check the explore findings still hold and the plan in session notes was sound, complete and actually implemented. You MUST call run_checks: it detects the language and package manager, runs the formatter check and the full test suite, and fails any changed source file without unit tests. The harness will not let verify end until run_checks passes on the current code; a run_checks failure is an automatic FAIL. Missing unit tests for new or changed logic are a FAIL even if everything else passes. Scrutinize every changed line against the highest standard: correctness and edge cases (nil, empty, errors, concurrency), broken or unupdated callers, missing tests for new logic; DRY — duplicated logic or an existing helper that was re-implemented; YAGNI — speculative abstractions, unused parameters, config or flexibility nobody asked for; clean and compact — dead code, leftover debug output, needless indirection, code that could be shorter without losing clarity; comments — verbose, redundant or narrating comments are defects, a comment may only state a non-obvious why; and scope — any unrequested change. Every finding must cite path:line and say exactly what to change. Do not soften, pad or praise; do not let a finding go because it is small. End with a verdict: PASS only if run_checks passed and you found nothing, otherwise FAIL with a numbered list ranked by severity. On FAIL, record the list with append_session_notes and call switch_mode(\"plan\", ...) so it gets fixed; never request write directly.\n")
	case AutoMode:
