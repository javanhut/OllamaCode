...
// UnwrapResponseEnvelope handles the exact {"response":"..."} transport shape
// emitted by some native tool-capable model templates on UNCONSTRAINED turns.
// Unlike UnwrapConstrainedProse it rejects bare JSON strings and objects with
// any additional field, so an ordinary JSON answer is never rewritten.
